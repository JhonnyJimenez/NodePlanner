...
class GraficadorVisual(QGraphicsView):
	...
	def interfaz(self):
		...
		self.setRenderHints(
							QPainter.RenderHint.Antialiasing | QPainter.RenderHint.HighQualityAntialiasing |
	...
	def clic_central_presionado(self, evento: QMouseEvent):
		# localPos y screenPos en PyQt5 fueron descontinuados en PyQt6.
		soltado_falso_del_mouse = QMouseEvent(
												QEvent.Type.MouseButtonRelease, evento.localPos(),
												evento.screenPos(), Qt.MouseButton.LeftButton,
		...
		evento_falso_del_mouse = QMouseEvent(
												evento.type(), evento.localPos(), evento.screenPos(),
