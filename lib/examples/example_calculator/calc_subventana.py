...
from lib.nodeeditor.GraficosVista import MODO_DIBUJO, MODO_REDIRECCION
...
class SubVenCalc(EditordeNodos):
	...
	def obtener_clase_del_nodo_de_datos(self, data):
		if 'Código op' not in data: return None
		return obtener_clase_del_codigo_op(data['Código op'])
	...
	def init_menú_contextual_de_nodos(self):
	...
	def soltar(self, event):
		if event.mimeData().hasFormat(LISTBOX_MIMETYPE):
			...
			try:
				...
				self.escena.historial.almacenar_historial("Creación de un nodo: %s" % nodo.__class__.__name__, set_modified =True)
	...
	def control_menú_contextual_de_los_nodos(self, event):
		...
		menu_contextual = QMenu(self)
		opción_indefinido = menu_contextual.addAction("Marcar indefinido")
		opción_descendencia_indefinido = menu_contextual.addAction("Marcar descendencia indefinido")
		opción_inválido = menu_contextual.addAction("Marcar inválido")
		opción_desmarcar_inválido = menu_contextual.addAction("Desmarcar invalido")
		opción_evaluado = menu_contextual.addAction("Marcar evaluado")
		...
		if seleccionado and acción == opción_indefinido: seleccionado.marcarIndefinido()
		if seleccionado and acción == opción_descendencia_indefinido: seleccionado.marcarDescendenciaIndefinido()
		if seleccionado and acción == opción_inválido: seleccionado.marcarInvalido()
		if seleccionado and acción == opción_desmarcar_inválido: seleccionado.marcarInvalido(False)
	...
	def control_menú_contextual_de_línea(self, event):
		...
		menu_contextual = QMenu(self)
		opción_bezier = menu_contextual.addAction("Línea bezier")
		opción_recta = menu_contextual.addAction("Línea recta")
		acción = menu_contextual.exec_(self.mapToGlobal(event.pos()))
		...
		if seleccionado and acción == opción_bezier: seleccionado.tipo_de_conexion = bezier
		if seleccionado and acción == opción_recta: seleccionado.tipo_de_conexion = recta
	...
	def control_menu_contextual_del_nuevo_nodo(self, event):
		...
		menu_contextual = self.init_menú_contextual_de_nodos()
		...
		if accion is not None:
			...
			if self.escena.obtener_vista().modo == MODO_DIBUJO:
				...
				zocalo_objetivo = self.definir_zocalo_objetivo_en_el_nodo(self.escena.obtener_vista().dibujado.zocalo_inicial_de_dibujado.es_salida, nuevo_calcnodo)
